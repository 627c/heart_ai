...
import matplotlib.pyplot as plt
from heart_2C.heart_myunet.segmentation_10_5_2 import _2_a,_2_rd,_2_rm,_2_ru,_2_ld,_2_lm,_2_lu
from heart_3C.heart_myunet.segmentation_10_5_2 import _3_a,_3_rd,_3_rm,_3_ru,_3_ld,_3_lm,_3_lu
from heart_4C.heart_myunet.segmentation_10_5_2 import _4_a,_4_rd,_4_rm,_4_ru,_4_ld,_4_lm,_4_lu
print(_3_ru)
...
def plot_bulls_eye(data):
    ...
    segment_angles_outer=np.linspace(0,2*np.pi,13)
    for i in range(12):
        ...
        radii_outer[1]=radii_outer[0]+0.05
        for t in range(5):
        ...
        #                 color=cmap(norm(data[idx])), edgecolor=None)
            ...
            idx += 1
            radii_outer[0]=radii_outer[0]+0.05
            radii_outer[1]=radii_outer[1]+0.05
        radii_outer[0]=0.75
        radii_outer[1]=radii_outer[0]+0.05
    ...
    segment_angles_middle=np.linspace(0,2*np.pi,13)
    for i in range(12):
        if idx >= len(data): break
        theta_start = segment_angles_middle[i]
        theta_end = segment_angles_middle[i+1]
        next_data = data[idx+1] if idx+1 < len(data) else data[idx]
        radii_middle[1]=radii_middle[0]+0.05
        for t in range(5):
        ...
        #                 color=cmap(norm(data[idx])), edgecolor=None)
            ...
            # ax.text(theta_start+(i//3+t/10)*(np.pi)/10, r_mid, f"{idx//10+1}_{idx%10+1}", color='black', ha='center', va='center', fontsize=8)
            idx += 1
            radii_middle[0]=radii_middle[0]+0.05
            radii_middle[1]=radii_middle[1]+0.05
        radii_middle[0]=0.5
        radii_middle[1]=radii_middle[0]+0.05
        # ax.text(theta_mid, 0.625, f"{i+13}", color='black', ha='center', va='center', fontsize=10)
    # 第三圈：4块
    radii_inner = [0.25, 0.5]
    segment_angles_inner=np.linspace(-np.pi/12,2*np.pi-np.pi/12,9)
    for i in range(8):
        if idx >= len(data): break
        theta_start = segment_angles_inner[i]
        theta_end = segment_angles_inner[i+1]
        next_data = data[idx+1] if idx+1 < len(data) else data[idx]
        radii_inner[1]=radii_inner[0]+0.05
        for t in range(5):
        ...
        #                 color=cmap(norm(data[idx])), edgecolor=None)
            ...
            # ax.text(theta_start+(i//3+t/10)*(np.pi)/10, r_mid, f"{idx//10+1}_{idx%10+1}", color='black', ha='center', va='center', fontsize=8)
            idx += 1
            radii_inner[0]=radii_inner[0]+0.05
            radii_inner[1]=radii_inner[1]+0.05
        radii_inner[0]=0.25
        radii_inner[1]=radii_inner[0]+0.05
        # ax.text(theta_mid, 0.375, f"{i+25}", color='black', ha='center', va='center', fontsize=10)
    # 中心圆：1块
    radii_center = [0, 0.25]
    ...
    for i in range(5):
        ...
        radii_center[0]=0
        radii_center[1]=radii_center[0]+0.125
        # ax.text(theta_mid, 0.375, f"{i+33}", color='black', ha='center', va='center', fontsize=10)
    # if idx < len(data):
    #     radii_center=[0,0.25]
    #     r_center = 0.25
    ...
    plt.savefig('/data/stu1/liuanqi/vision/_bull_10_5_2 flow.png')
    plt.show()
