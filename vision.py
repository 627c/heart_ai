...
def plot_bulls_eye(data):
    ...
    segment_angles_outer=np.linspace(0,2*np.pi,61)
    for i in range(60):
        ...
        ax.text(theta_mid, r_mid, f"{idx//10+1}_{idx%10+1}", color='black', ha='center', va='center', fontsize=8)
        idx += 1

    # 第二圈：6块
    radii_middle = [0.5, 0.75]
    segment_angles_middle=np.linspace(0,2*np.pi,61)
    for i in range(60):
        if idx >= len(data): break
        ...
        ax.text(theta_mid, r_mid, f"{idx//10+1}_{idx%10+1}", color='black', ha='center', va='center', fontsize=5)
        idx += 1

    # 第三圈：4块
    radii_inner = [0.25, 0.5]
    segment_angles_inner=np.linspace(0,2*np.pi,41)
    for i in range(40):
        if idx >= len(data): break
        ...
        # ax.fill_between(np.linspace(theta_start, theta_end, 100), radii_inner[0], radii_inner[1],
        #                 color=cmap(norm(data[idx])), edgecolor=None)
        theta_mid = (theta_start + theta_end) / 2
        r_mid = (radii_inner[0] + radii_inner[1]) / 2
        ax.text(theta_mid, r_mid, f"{idx//10+1}_{idx%10+1}", color='black', ha='center', va='center', fontsize=4)
        idx += 1
    ...
    plt.savefig('/data/stu1/liuanqi/vision/Blood flow.png')
    plt.show()
