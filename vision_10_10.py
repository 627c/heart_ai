...
import matplotlib.pyplot as plt
from heart_2C.heart_myunet.segmentation_10_10 import _2_a,_2_rd,_2_rm,_2_ru,_2_ld,_2_lm,_2_lu
from heart_3C.heart_myunet.segmentation_10_10 import _3_a,_3_rd,_3_rm,_3_ru,_3_ld,_3_lm,_3_lu
from heart_4C.heart_myunet.segmentation_10_10 import _4_a,_4_rd,_4_rm,_4_ru,_4_ld,_4_lm,_4_lu
print(_3_ru)
segment_values=[0]*170
for i in range (10):
    segment_values[i]=_2_rd[i]
for i in range(10,20):
    segment_values[i]=_3_rd[i-10]
for i in range(20,30):
    segment_values[i]=_4_ld[i-20]
for i in range(30,40):
    segment_values[i]=_2_ld[i-30]
for i in range(40,50):
    segment_values[i]=_3_ld[i-40]
for i in range(50,60):
    segment_values[i]=_4_rd[i-50]
for i in range(60,70):
    segment_values[i]=_2_rm[i-60] 
for i in range(70,80):
    segment_values[i]=_3_rm[i-70]
for i in range(80,90):
    segment_values[i]=_4_lm[i-80]
for i in range(90,100):
    segment_values[i]=_2_lm[i-90] 
for i in range(100,110):
    segment_values[i]=_3_lm[i-100] 
for i in range(110,120):
    segment_values[i]=_4_rm[i-110]
for i in range(120,130):
    segment_values[i]=_2_ru[i-120] 
for i in range(130,140):
    segment_values[i]=(_3_ru[i-130] + _4_lu[i-130]) /2
for i in range(140,150):
    segment_values[i]=_2_lu[i-140]
for i in range(150,160):
    segment_values[i]=(_3_lu[i-150] +_4_ru[i-150]) /2
for i in range(160,170):
    segment_values[i]=(_2_a[i-160]+_3_a[i-160]+_4_a[i-160])/3                                                
# _1=_2_1
# _2=_3_1
# _3=_4_5
# _4=_2_5
...
def plot_bulls_eye(data):
    ...
    def fill_gradient(ax,theta_start, theta_end, r_inner, r_outer, color_start, color_end):
        ...
        for i in range(500):  # Increase the number of interpolation steps for smoother transition
            t0 = theta_start + (theta_end - theta_start) * (i / 100)
            t1 = theta_start + (theta_end - theta_start) * ((i + 1) / 100)
            c = cmap(norm(color_start + (color_end - color_start) * (i / 100)))
            ax.fill_between([t0, t1], r_inner, r_outer, color=c, edgecolor=None,linewidth=0)

    # 最外圈：6块
    radii_outer = [0.75, 1.0]
    segment_angles_outer=np.linspace(0,2*np.pi,7)
    for i in range(6):
        ...
        radii_outer[1]=radii_outer[0]+0.025
        for t in range(10):
        ...
        #                 color=cmap(norm(data[idx])), edgecolor=None)
            ...
            idx += 1
            radii_outer[0]=radii_outer[0]+0.025
            radii_outer[1]=radii_outer[1]+0.025
        radii_outer[0]=0.75
        radii_outer[1]=radii_outer[0]+0.025
    ...
    segment_angles_middle=np.linspace(0,2*np.pi,7)
    for i in range(6):
        if idx >= len(data): break
        theta_start = segment_angles_middle[i]
        theta_end = segment_angles_middle[i+1]
        next_data = data[idx+1] if idx+1 < len(data) else data[idx]
        radii_middle[1]=radii_middle[0]+0.025
        for t in range(10):
        ...
        #                 color=cmap(norm(data[idx])), edgecolor=None)
            ...
            # ax.text(theta_start+(i//3+t/10)*(np.pi)/10, r_mid, f"{idx//10+1}_{idx%10+1}", color='black', ha='center', va='center', fontsize=8)
            idx += 1
            radii_middle[0]=radii_middle[0]+0.025
            radii_middle[1]=radii_middle[1]+0.025
        radii_middle[0]=0.5
        radii_middle[1]=radii_outer[0]+0.025
        ax.text(theta_mid, 0.625, f"{i+7}", color='black', ha='center', va='center', fontsize=10)
    # 第三圈：4块
    radii_inner = [0.25, 0.5]
    segment_angles_inner=np.linspace(-np.pi/12,2*np.pi-np.pi/12,5)
    for i in range(4):
        if idx >= len(data): break
        theta_start = segment_angles_inner[i]
        theta_end = segment_angles_inner[i+1]
        next_data = data[idx+1] if idx+1 < len(data) else data[idx]
        radii_inner[1]=radii_inner[0]+0.025
        for t in range(10):
        ...
        #                 color=cmap(norm(data[idx])), edgecolor=None)
            ...
            # ax.text(theta_start+(i//3+t/10)*(np.pi)/10, r_mid, f"{idx//10+1}_{idx%10+1}", color='black', ha='center', va='center', fontsize=8)
            idx += 1
            radii_inner[0]=radii_inner[0]+0.025
            radii_inner[1]=radii_inner[1]+0.025
        radii_inner[0]=0.25
        radii_inner[1]=radii_outer[0]+0.025
        ax.text(theta_mid, 0.375, f"{i+13}", color='black', ha='center', va='center', fontsize=10)
    # 中心圆：1块
    radii_center = [0, 0.25]
    segment_angles_middle=np.linspace(0,2*np.pi,11)
    for i in range(10):
        if idx >= len(data): break
        theta_start = segment_angles_middle[i]
        theta_end = segment_angles_middle[i+1]
        ...
        # ax.fill_between(np.linspace(theta_start, theta_end, 100), radii_middle[0], radii_middle[1],
        #                 color=cmap(norm(data[idx])), edgecolor=None)
        theta_mid = (theta_start + theta_end) / 2
        r_mid = (radii_center[0] + radii_center[1]) / 2
        ax.text(theta_mid, r_mid, f"{idx//10+1}_{idx%10+1}", color='black', ha='center', va='center', fontsize=6)
        idx += 1
    # if idx < len(data):
    #     radii_center=[0,0.25]
    #     r_center = 0.25
    ...
    plt.title("Pixel-level Bull's Eye Plot Plot")
    plt.savefig('/data/stu1/liuanqi/vision/_bull_10_10.png')
    plt.show()
